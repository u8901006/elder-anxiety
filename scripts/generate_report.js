...
1. 從提供的醫學文獻中，篩選出最具臨床意義與研究價值的老年焦慮相關論文
...
function safeParseJSON(text) {
  ...
  try {
    ...
    if (jsonStart >= 0 && jsonEnd > jsonStart) {
      ...
      if (lastValidEnd > 0) {
        const repaired = candidate.slice(0, lastValidEnd + 1);
        try {
          return JSON.parse(repaired);
        } catch (e2) {
          console.error(`[WARN] Repaired JSON parse also failed: ${e2.message}`);
        }
      }
      ...
      try {
        ...
        console.error(`[WARN] Candidate JSON parse failed: ${e3.message}`);
      }
    }
...
async function analyzePapers(apiKey, papersData) {
  ...
  const dateStr = papersData.date || now.toISOString().slice(0, 10);
  const paperCount = papersData.count || 0;
  const papersText = JSON.stringify(papersData.papers || [], null, 2);

  const prompt = `以下是 ${dateStr} 從 PubMed 抓取的最新老年焦慮相關文獻（共 ${paperCount} 篇）。

請進行以下分析，並以 JSON 格式回傳（不要用 markdown code block）：

{
  "date": "${dateStr}",
  "market_summary": "1-2句話總結今天文獻的整體趨勢與亮點，聚焦於老年焦慮領域",
  "top_picks": [
    {
      "rank": 1,
      "title_zh": "中文標題",
...
每篇 paper 的 tags 請從以下選擇（可多選）：${TAG_OPTIONS.join("、")}
記住：回傳純 JSON，不要用 \`\`\`json\`\`\` 包裹。務必確保 JSON 格式完整正確，所有字串正確轉義。`;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        if (resp.statusCode >= 400) {
          console.error(
            `[ERROR] HTTP ${resp.statusCode}: ${resp.body.slice(0, 300)}`
          );
          break;
        }
...
function generateHtml(analysis) {
  ...
  const weekday =
    parts.length === 3
      ? weekdays[new Date(dateStr).getDay()]
      : "";

  const summary = escapeHtml(
    analysis.market_summary || "今日暫無新的老年焦慮相關文獻更新。"
  );
    ...
    topPicksHtml += `
        <div class="news-card featured">
          ...
          <div class="card-footer">
            ${tagsHtml}
            <a href="${escapeHtml(pick.url || "#")}" target="_blank" rel="noopener noreferrer">閱讀原文 &rarr;</a>
          </div>
        </div>`;
  }
  ...
  for (const paper of allPapers) {
    ...
    allPapersHtml += `
        <div class="news-card">
          ...
          <div class="card-footer">
            ${tagsHtml}
            <a href="${escapeHtml(paper.url || "#")}" target="_blank" rel="noopener noreferrer">PubMed &rarr;</a>
          </div>
        </div>`;
  }
...
<title>Elder Anxiety Daily &middot; 老年焦慮文獻日報 &middot; ${dateDisplay}</title>
<meta name="description" content="${dateDisplay} 老年焦慮文獻日報，由 AI 自動彙整 PubMed 最新論文"/>
<style>
  :root { --bg: #f6f1e8; --surface: #fffaf2; --line: #d8c5ab; --text: #2b2118; --muted: #766453; --accent: #8c4f2b; --accent-soft: #ead2bf; --card-bg: #fffdf8; }
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .link-text { font-size: 15px; font-weight: 700; color: var(--text); flex: 1; }
...
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>Elder Anxiety Daily &middot; 老年焦慮文獻日報</h1>
  ...
  ${topicBarsHtml ? `<div class="topic-section section"><div class="section-title"><span class="section-icon">📊</span>主題分布</div>${topicBarsHtml}</div>` : ""}
  ...
  <div class="links-section">
    <a href="https://www.leepsyclinic.com/" class="link-card" target="_blank" rel="noopener noreferrer">
      <span class="link-icon">🏥</span>
      <span class="link-text">李政洋身心診所首頁</span>
      <span class="link-arrow">&rarr;</span>
    </a>
    <a href="https://blog.leepsyclinic.com/" class="link-card" target="_blank" rel="noopener noreferrer">
      <span class="link-icon">📬</span>
      <span class="link-text">訂閱電子報</span>
      <span class="link-arrow">&rarr;</span>
    </a>
    <a href="https://buymeacoffee.com/CYlee" class="link-card" target="_blank" rel="noopener noreferrer">
      <span class="link-icon">☕</span>
...
async function main() {
  ...
  if (!papersData || !papersData.papers || papersData.papers.length === 0) {
    ...
    analysis = {
      ...
      market_summary:
        "今日 PubMed 暫無新的老年焦慮相關文獻更新。請明天再查看。",
      top_picks: [],
      all_papers: [],
      keywords: [],
      topic_distribution: {},
