...
const htmlFiles = fs
  ...
  .filter((f) => f.startsWith("elder-anxiety-") && f.endsWith(".html"))
  .sort()
  .reverse();
...
let links = "";
const displayFiles = htmlFiles.slice(0, 30);
for (const f of displayFiles) {
  const date = f.replace("elder-anxiety-", "").replace(".html", "");
...
<title>Elder Anxiety Daily · 老年焦慮文獻日報</title>
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .link-text { font-size: 14px; font-weight: 600; color: var(--text); }
...
<div class="container">
  <div class="logo">👴</div>
  <h1>Elder Anxiety Daily</h1>
  <p class="subtitle">老年焦慮文獻日報 · 每日自動更新</p>
  ...
  <footer>
    <p>Powered by PubMed + NVIDIA AI · <a href="https://github.com/u8901006/elder-anxiety">GitHub</a></p>
  </footer>
</div>
</body>
</html>`;

fs.writeFileSync(path.join(DOCS_DIR, "index.html"), index, "utf-8");
